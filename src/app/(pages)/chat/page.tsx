      ...
      <div className="absolute bottom-0 left-0 right-0 p-4 flex flex-row gap-2">
        <div className="flex flex-row gap-2">
          <button className="rounded-full min-w-9 w-9 h-9 flex items-center justify-center self-end hover:cursor-pointer dark:hover:bg-neutral-600/30 hover:bg-neutral-900/30">
            <ImagePlay
              size={14}
              color={resolvedTheme === "dark" ? "#d4d4d4" : "#0a0a0a"}
              className="-translate-x-px translate-y-[0.5px] scale-120 group-hover:opacity-100"
            />
          </button>
          <button className="rounded-full min-w-9 w-9 h-9 flex items-center justify-center self-end hover:cursor-pointer dark:hover:bg-neutral-600/30 hover:bg-neutral-900/30">
            <Smile
              size={14}
              color={resolvedTheme === "dark" ? "#d4d4d4" : "#0a0a0a"}
              className="scale-120 group-hover:opacity-100"
            />
          </button>
        </div>
        <textarea
          ...
          onInput={(e) => {
            ...
            target.style.height = `${Math.min(target.scrollHeight, 72)}px`; // 72px ≈ 3 lines
          }}
        />
        <button className="rounded-full min-w-9 w-9 h-9 flex items-center justify-center self-end hover:cursor-pointer dark:hover:bg-neutral-600/30 hover:bg-neutral-900/30">
          <Send
            size={14}
            color={resolvedTheme === "dark" ? "#d4d4d4" : "#0a0a0a"}
            className="-translate-x-[1.5px] translate-y-[1.5px] scale-120 group-hover:opacity-100"
          />
        </button>
      </div>
    </div>
